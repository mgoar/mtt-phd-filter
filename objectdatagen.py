...
def objectdatagen(ground_truth: modelgen.modelgen, motion_model: motionmodel.motionmodel):
    ...
    for i in np.arange(0, ground_truth.n_births):
        ...
        for k in np.arange(ground_truth.t_birth[i], np.min([ground_truth.t_death[i], K])).astype(int):
