...
class measmodel:
    ...
    def rangebearingmeasmodel(self, sigma_r: float, sigma_b: float, s: np.ndarray):
        ...
        # Measurement model Jacobian
        self.H = lambda x: np.array([np.pad([(x[0]-s[0])/rng(x), (x[1]-s[1])/rng(x)], (0, x.shape[0]-2)),
                                     np.pad([-(x[1]-s[1])/(rng(x)**2), (x[0]-s[0])/(rng(x)**2)], (0, x.shape[0]-2))])
